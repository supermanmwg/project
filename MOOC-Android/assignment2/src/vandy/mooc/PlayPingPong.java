   ...
   private final CyclicBarrier pingpongBarrier;
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
            ...
            try {
            	...
            	Log.i(TAG, "before barrier ");
            	pingpongBarrier.await();
            	Log.i(TAG, "after barrier ");
        ...
        public boolean handleMessage(Message reqMsg) {
            ...
            // appropriate code.
        	Log.i(TAG, "max iterations is " + mCurrentIterations);
            if (0 < mCurrentIterations) {
            	if(handlers[0] == reqMsg.getTarget())
            		mOutputStrategy.print("PING" + (mCurrentIterations/2) + "\n");
            	else if (handlers[1] == reqMsg.getTarget())
            		mOutputStrategy.print("PONG" + (mCurrentIterations/2) + "\n");
            	mCurrentIterations--;
            } else {
                // Shutdown the HandlerThread to the main PingPong
                // thread can join with it.
                // @@ TODO - you fill in here.
            	Log.i(TAG, "shutdown the thread!");
            	i++;
            	this.quit();
            	if(i == 2) return true;
            }
    ...
    public void run() {
        ...
        try {
			...
			Log.i(TAG, "Catch ping thread interrupt");
			e1.printStackTrace();
		}
        try {
			pongThread.join();
			...
			// TODO Auto-generated catch block
			Log.i(TAG, "Catch pong thread interrupt");
			e1.printStackTrace();
		}
        ...
        // @@ TODO - you fill in here.
 /*     try {
    	  Log.i(TAG, "Before the last barrier");
        	pingpongBarrier.await();
        
          Log.i(TAG, "After the last barrier");
